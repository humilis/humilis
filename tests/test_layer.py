...
def test_create_layer_object(test_environment, test_vpc_layer):
    ...
    assert layer.name == "{}-vpc".format(test_environment.name)
...
def test_get_section_files(test_vpc_layer):
    assert len(test_vpc_layer.get_section_files('resources')) == 2
    assert len(test_vpc_layer.get_section_files('meta')) == 1
    assert len(test_vpc_layer.get_section_files('invalid')) == 0


def test_create_and_delete_layer(cf, test_roles_layer):
    # Make sure the stack wasn't there already
    assert not cf.stack_exists(test_roles_layer.name)
    ...
    assert cf.stack_ok(test_roles_layer.name)
...
def test_load_section(test_vpc_layer):
    files = test_vpc_layer.get_section_files('resources')
    data = test_vpc_layer.load_section('resources', files)
...
def test_create_layer_lacking_dependencies(cf, test_instance_layer):
    """Attempts to create a stack lacking dependencies: exception"""
    assert not cf.stack_exists(test_instance_layer.name)
...
def test_create_layer_absent_section_dirs(cf, test_streams_layer):
    ...
    assert cf.stack_exists(test_streams_layer.name)
...
def test_create_dependant_layer(cf, test_vpc_layer, test_instance_layer):
    ...
    assert cf.stack_ok(test_vpc_layer.name)
    test_instance_layer.create()
    assert cf.stack_ok(test_instance_layer.name)
    test_instance_layer.delete()
    assert not cf.stack_exists(test_instance_layer.name)
    test_vpc_layer.delete()
    assert not cf.stack_exists(test_vpc_layer.name)
...
def test_create_namedinstance_stack(cf, test_vpc_layer,
    ...
    assert cf.stack_ok(test_named_instance_layer.name)
    test_named_instance_layer.delete()
    assert not cf.stack_exists(test_named_instance_layer.name)
    test_vpc_layer.delete()
    assert not cf.stack_exists(test_vpc_layer.name)
